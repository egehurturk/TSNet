...
class CausalConv1d(torch.nn.Conv1d):
  """
  A causal 1D convolution block with dilations
  """
...
class WaveNet(nn.Module):
  """
  The WaveNet, see https://arxiv.org/pdf/1609.03499.pdf
  """
  def __init__(self, num_channels, dilation_depth, num_repeat, kernel_size=2):
      super(WaveNet, self).__init__()
      dilations = [2 ** d for d in range(dilation_depth)] * num_repeat # dilations per layer
...
def train_error_to_signal(y, y_pred):
    """
    Error to signal ratio with pre-emphasis filter:
    https://www.mdpi.com/2076-3417/10/3/766/htm
    """
    y, y_pred = train_pre_emphasis_filter(y), train_pre_emphasis_filter(y_pred)
    return (y - y_pred).pow(2).sum(dim=2) / (y.pow(2).sum(dim=2) + 1e-10)


def train_pre_emphasis_filter(x, coeff=0.95):
    return torch.cat((x[:, :, 0:1], x[:, :, 1:] - coeff * x[:, :, :-1]), dim=2)


class DistNet(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = train_error_to_signal(y[:, :, -y_pred.size(2) :], y_pred).mean()
        logs = {"loss": loss}
